...
from pytket.circuit import Command, Conditional, Circuit, CircBox, BarrierOp
from pytket import unit_id
from typing import Any

from .._tket.passes import BasePass, CustomPass

def extract_cond(cmd: Command) -> tuple[int, list[Any]]:
    if isinstance(cmd.op, Conditional):
        return (cmd.op.value, cmd.args[:cmd.op.width])
    else:
        return None

def combine_conditionals(circuit: Circuit) -> Circuit:
    ...
    def emit_cond_box(top_circ, sub_circ, cond):
        cond_value = cond[0]
        cond_args = cond[1]
        ...
        if sub_circ.n_gates == 1:
            ...
            top_circ.add_gate(sub_cmd.op, sub_arg_list, 
                condition_bits=cond_args, condition_value=cond_value)
        else:
            new_circuit.add_gate(CircBox(sub_circ), sub_arg_list,
                condition_bits=cond_args, condition_value=cond_value)

    for idx, cmd in enumerate(circuit.get_commands()):
        ...
        if cond is not None:
            ...
            for arg in cmd.args[width:]:
                if arg not in sub_args:
                    ...
                    elif isinstance(arg, unit_id.QubitRegister):
                        sub_circ.add_q_register(arg)
                    elif isinstance(arg, unit_id.BitRegister):
                        sub_circ.add_c_register(arg)
                    else:
                        raise ValueError("Unknown arg type")
                    sub_args.add(arg)

            if isinstance(cmd.op.op, BarrierOp):
                sub_circ.add_barrier(cmd.args[width:])
            else:
                sub_circ.add_gate(cmd.op.op, cmd.args[width:])
            curr_cond = cond
        else:
            if isinstance(cmd.op, BarrierOp):
