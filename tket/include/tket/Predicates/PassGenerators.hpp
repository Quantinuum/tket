 ...
 */
TKET_EXPORT PassPtr
gen_auto_rebase_pass(const OpTypeSet& allowed_gates, bool allow_swaps = false);
...
TKET_EXPORT PassPtr
gen_euler_pass(const OpType& q, const OpType& p, bool strict = false);
 ...
 */
TKET_EXPORT PassPtr
gen_flatten_relabel_registers_pass(const std::string& label);
/**
...
TKET_EXPORT PassPtr
gen_default_mapping_pass(const Architecture& arc, bool delay_measures = true);
 ...
 * replacement circuit */
TKET_EXPORT PassPtr
gen_user_defined_swap_decomp_pass(const Circuit& replacement_circ);
 ...
 */
TKET_EXPORT PassPtr
DecomposeTK2(const Transforms::TwoQbFidelities& fid, bool allow_swaps = true);
...
TKET_EXPORT PassPtr
gen_optimise_phase_gadgets(CXConfigType cx_config = CXConfigType::Snake);

/* generates an optimisation pass that converts a circuit into Pauli
gadgets and optimises them using techniques from
https://arxiv.org/abs/1906.01734 */
TKET_EXPORT PassPtr
gen_pairwise_pauli_gadgets(CXConfigType cx_config = CXConfigType::Snake);
 ...
 */
TKET_EXPORT PassPtr
PauliSquash(Transforms::PauliSynthStrat strat, CXConfigType cx_config);
